...
-- Lượng khách theo tab: 1=giờ, 2=thứ (key day), 3=ngày (key date)
-- Đếm appointment (không hủy)
...
CREATE PROCEDURE dbo.usp_GetCustomerTraffic
    ...
    @FromDate DATE,
    @ToDate   DATE,
    @Tab      TINYINT   -- 1=hour, 2=weekday, 3=date
AS
BEGIN
    SET NOCOUNT ON;
    ...
    ELSE
    BEGIN
        -- Theo ngày lịch (frontend key: date)
        SELECT

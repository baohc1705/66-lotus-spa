...
namespace _66SMS.Application.PaymentService.Cashier.Commands.VnPayReturn
{
        ...
        : IRequestHandler<VnPayReturnCommand, Result<VnPayReturnDto>>
    {
        public async Task<Result<VnPayReturnDto>> Handle(VnPayReturnCommand request, CancellationToken cancellationToken)
        {
            ...
            if (appointment == null) return Result<VnPayReturnDto>.NotFound(AppointmentPaymentConst.MSG_PAYMENT_ORDER_NOT_FOUND, ErrorCodes.ERR_PAYMENT_ORDER_NOT_FOUND);

  ...
  return (
    <div className="flex items-center gap-4">
      <button
        ...
        className={[
          ...
          displaySrc
            ...
            : 'border-2 border-dashed border-adminGray-300 bg-adminGray-50 hover:border-adminGreen-600 hover:bg-adminGreen-600/5',
        ].join(' ')}
      >
        {displaySrc ? (
          <>
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center gap-1 bg-black/50 opacity-0 transition-opacity group-hover:opacity-100">
              ...
              <span className="text-xs font-semibold text-white">Thay ảnh</span>
            </div>
          </>
        ) : (
          <div className="flex h-full w-full flex-col items-center justify-center gap-2 text-adminGray-400 transition-colors group-hover:text-adminGreen-600">
            {shape === 'circle' ? <User className="h-8 w-8" /> : <ImageIcon className="h-8 w-8" />}
            <span className="px-2 text-center text-2xs font-medium leading-tight">{label}</span>
          </div>
        )}
      </button>
      ...
      <div className="space-y-1">
        {displaySrc ? (
          <p className="text-xs font-medium text-state-success-text">✓ Đã chọn ảnh</p>
        ) : (
          <p className="text-xs text-adminGray-600">Nhấp vào ô ảnh để tải lên</p>
        )}
        <p className="text-xs text-adminGray-400">JPG, PNG, WEBP · Tối đa 5MB</p>
      </div>
    </div>
  )
}

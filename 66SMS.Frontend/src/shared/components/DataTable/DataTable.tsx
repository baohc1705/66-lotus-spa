                    ...
                    {header.column.getCanResize() && (
                      <div
                        ...
                        className={`absolute top-0 right-0 h-full w-1.5 cursor-col-resize hover:bg-white/30 touch-none select-none ${
                          header.column.getIsResizing() ? 'bg-white/40' : 'bg-transparent'
                        }`}
                      />
                    )}
                  </TableHead>
        ...
        <TableBody>
          {isLoading ? (
            ...
            table.getRowModel().rows.map((row) => (
              <Fragment key={row.id}>
                <TableRow
                  data-state={row.getIsExpanded() ? 'expanded' : row.getIsSelected() ? 'selected' : undefined}
                  className={cn(
                    onRowClick ? 'cursor-pointer group' : '',
                    row.getIsExpanded() ? 'border-x-2 border-t-2 border-lotus-leaf relative z-10 bg-lotus-leaf-light/50' : ''
                  )}
                  onClick={() => {

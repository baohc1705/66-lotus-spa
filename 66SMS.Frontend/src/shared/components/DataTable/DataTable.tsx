                ...
                <TableRow key={`skel-${i}`}>
                  {table.getVisibleFlatColumns().map((col, j) => (
                    <TableCell key={j} style={{ width: col.getSize() }}>
                      <Skeleton className="h-4 w-3/4 max-w-28 rounded" />
                    </TableCell>
                  ))}
                </TableRow>
              ))
              ...
              table.getRowModel().rows.map((row) => (
                <Fragment key={row.id}>
                  ...
                  {row.getIsExpanded() && renderSubComponent && (
                    <TableRow className="bg-adminGray-50/30 hover:bg-adminGray-50/30 !border-x-2 !border-b-2 border-t-0 border-adminGreen-600 relative z-10 shadow-xs">
                      <TableCell

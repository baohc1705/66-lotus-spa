...
import {
  ...
  ArrowUpDown, Package, ArrowUp, ArrowDown, X
} from 'lucide-react'
...
export function ProductListPage() {
  ...
  const { data: productsResult, isLoading, isFetching } = useProducts({
    pageIndex,
    pageSize,
  ...
  const currentPageIds = useMemo(() => products.map(p => p.id).filter((id): id is number => id !== undefined), [products])
  ...
  const columns = useMemo<ColumnDef<ProductDto>[]>(() => [
    ...
    },
    {
      accessorKey: 'code',
      header: 'Mã SP',
      cell: ({ row }) => <span className="text-lotus-deep/80 font-medium">{row.original.code ?? '—'}</span>,
      size: 100,
    },
    {
      accessorKey: 'name',
        ...
        return (
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-lg bg-stone-100 flex items-center justify-center shrink-0 overflow-hidden">
              {primaryImage ? (
                <img src={primaryImage} alt="" className="w-8 h-8 object-cover" />
              ) : (
    ...
    },
    {
      ...
      header: 'Danh mục',
      cell: ({ row }) => <span className="text-lotus-deep/70">{row.original.categoryName ?? '—'}</span>,
      size: 120,
    },
    {
      accessorKey: 'stockQuantity',
      header: () => (
        <button onClick={() => handleSort('stockquantity')} className="flex items-center gap-1.5 hover:text-lotus-leaf transition-colors">
      ...
      cell: ({ row }) => {
        ...
        return (
          <span className={`font-semibold ${isLowStock ? 'text-red-500' : 'text-lotus-deep'}`}>
            {stock}
          </span>
        )
      },
    ...
    },
    {
      accessorKey: 'unit',
      header: 'Đơn vị',
      cell: ({ row }) => <span className="text-lotus-deep/70">{row.original.unit ?? '—'}</span>,
      size: 80,
    },
    {
      ...
      cell: ({ row }) => (
        <span className="font-semibold text-lotus-deep">
          {row.original.sellingPrice != null ? new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(row.original.sellingPrice) : '—'}
        </span>
      ),
      size: 120,
    },
    {
      accessorKey: 'status',
      header: 'Trạng thái',
      cell: ({ row }) => <StatusBadge status={row.original.status?.toString()} statusMap={PRODUCT_STATUS_MAP} />,
      size: 120,
    },
    {
      id: 'actions',
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent>
                ...
                {/* <PermissionGate resource="products" action="delete" role="admin"> */}
                  <DropdownMenuSeparator />
                  <DropdownMenuItem variant="destructive" onClick={() => setDeleteTarget(prod)}>
        ...
        <DataTable
          ...
          emptyState={
            <div className="flex flex-col items-center gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-lotus-deep">Chưa có sản phẩm</p>
                <p className="text-[12px] text-lotus-stone mt-0.5">Thêm sản phẩm mới để bắt đầu quản lý kho.</p>
              </div>
              {/* <PermissionGate resource="products" action="create"> */}

...
export function ProductDetailExpanded({ productId, onEdit }: ProductDetailExpandedProps) {
  ...
  if (!product) {
    return <div className="p-6 text-center text-lotus-stone text-sm bg-stone-50/30">Không tìm thấy thông tin sản phẩm</div>
  }
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex items-center gap-4 pb-4 border-b border-stone-200/50">
          <div className="w-16 h-16 rounded-xl bg-lotus-cream/50 flex items-center justify-center shrink-0 overflow-hidden shadow-sm border border-stone-200/50 p-1">
            {primaryImage ? (
              <img src={primaryImage} alt={product.name ?? ''} className="w-full h-full object-cover rounded-lg" />
            ) : (
              <Package className="w-8 h-8 text-lotus-stone" />
            )}
          </div>
          <div className="flex-1 min-w-0">
            <h3 className="text-base font-bold text-lotus-deep truncate">{product.name ?? '—'}</h3>
            <p className="text-[12px] text-lotus-stone mt-0.5 font-medium">
              Mã: {product.code || '—'} · Danh mục: {product.categoryName || '—'}
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-0">
          <div className="flex flex-col">
            ...
            <DetailField label="Giá vốn" value={product.costPrice != null ? `${product.costPrice.toLocaleString()} ₫` : '—'} />
            <DetailField label="Giá bán" value={product.sellingPrice != null ? `${product.sellingPrice.toLocaleString()} ₫` : '—'} />
            <DetailField label="Tồn kho" value={product.stockQuantity?.toString()} />
            <DetailField label="Tồn kho tối thiểu" value={product.minStock?.toString()} />
          </div>
          <div className="flex flex-col">
        ...
        <div className="flex items-end justify-end mt-2 pt-4 border-t border-stone-100/80">
          {/* <PermissionGate resource="products" action="update"> */}
            <Button variant="admin" size="sm" onClick={() => onEdit?.(product)} className="bg-lotus-leaf hover:opacity-90 text-white shadow-sm h-8 px-4 text-[13px] gap-1.5 rounded-md transition-opacity">
...
function DetailField({ label, value }: { label: string; value: string | null | undefined }) {
  return (
    <div className="py-3.5 border-b border-stone-100/80 last:border-b-0 group">
      ...
      <p className="text-[13px] font-medium text-lotus-deep break-words whitespace-pre-wrap">{value || '—'}</p>
    </div>
  )
}

    ...
    return (
      <div className="card-glow bg-white/70 backdrop-blur-md rounded-admin p-6 border border-stone-200/30 h-[280px] flex flex-col justify-between">
        ...
        <div className="space-y-4 mt-6">
          <div className="h-6 w-full bg-stone-100 rounded animate-pulse" />
          <div className="h-6 w-full bg-stone-100 rounded animate-pulse" />
          <div className="h-6 w-full bg-stone-100 rounded animate-pulse" />
        </div>
      </div>
      ...
      <div className="mt-4 flex flex-col flex-1 justify-between">
        ...
        <div>
          <div className="flex justify-between items-baseline mb-1">
            <span className="text-lotus-admin-xs text-stone-400 font-bold uppercase tracking-wider">Tổng tiền thực chi</span>
            <span className="text-lg font-bold text-lotus-deep">{formatCurrency(displayTotal)}</span>
          </div>
          <div className="w-full h-3 rounded-full flex overflow-hidden bg-stone-100/50 mt-1.5 border border-stone-200/30">
            {payrollBase > 0 && <div className="bg-lotus-leaf h-full" style={{ width: `${basePct}%` }} />}
            {commission > 0 && <div className="bg-lotus-accent h-full" style={{ width: `${commPct}%` }} />}
            {refunds > 0 && <div className="bg-lotus-rose h-full" style={{ width: `${refundPct}%` }} />}
          </div>
        </div>
        ...
        <div className="space-y-3 mt-4">
          {items.map((item, idx) => {
            ...
            return (
              <div key={idx} className="flex items-center justify-between text-lotus-admin-md">
                <div className="flex items-center gap-2">
                  <span className={`w-2.5 h-2.5 rounded-sm ${item.color} shrink-0`} />
                  <Icon className="w-3.5 h-3.5 text-stone-400 shrink-0" />
                  <span className="text-lotus-deep font-semibold truncate max-w-[170px] sm:max-w-none">
                    {item.label}
                  </span>
                </div>
                <div className="flex items-center gap-2 text-right shrink-0">
                  <span className="font-bold text-lotus-deep">{formatCurrency(item.amount)}</span>
                  <span className="text-lotus-admin-xs font-bold text-stone-400 bg-stone-100/70 px-1.5 py-0.5 rounded">
                    {item.percent}%
                  </span>
                </div>
              </div>

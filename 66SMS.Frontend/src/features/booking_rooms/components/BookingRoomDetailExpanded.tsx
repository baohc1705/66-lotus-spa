...
export function BookingRoomDetailExpanded({ roomId, onEdit }: BookingRoomDetailExpandedProps) {
  ...
  if (!room) {
    return <div className="p-6 text-center text-adminGray-600 text-sm bg-adminGray-50/30">Không tìm thấy thông tin phòng dịch vụ</div>
  }
      ...
      <Tabs defaultValue="info" className="w-full flex-col">
        ...
        <div className="px-4 pt-2 sticky top-0 bg-adminGray-50/95 backdrop-blur-sm z-10">
          <TabsList className="h-10 border-b border-adminGray-100/80 justify-start rounded-none bg-transparent p-0 flex flex-nowrap overflow-x-auto overflow-y-hidden hide-scrollbar">
            <TabsTrigger value="info" className="relative h-10 rounded-none border-0 border-b-2 border-transparent bg-transparent px-3 pb-2 pt-2 text-sm font-medium text-adminGray-600 hover:text-adminGreen-600/80 data-[state=active]:border-adminGreen-600 data-[state=active]:text-adminGreen-600 data-[state=active]:bg-transparent data-[state=active]:shadow-none focus-visible:ring-0 focus-visible:outline-none whitespace-nowrap transition-colors">Thông tin chung</TabsTrigger>
            <TabsTrigger value="positions" className="relative h-10 rounded-none border-0 border-b-2 border-transparent bg-transparent px-3 pb-2 pt-2 text-sm font-medium text-adminGray-600 hover:text-adminGreen-600/80 data-[state=active]:border-adminGreen-600 data-[state=active]:text-adminGreen-600 data-[state=active]:bg-transparent data-[state=active]:shadow-none focus-visible:ring-0 focus-visible:outline-none whitespace-nowrap transition-colors">Danh sách vị trí ({positions.length})</TabsTrigger>
          </TabsList>
        </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                <h3 className="text-base font-bold text-adminInk truncate">{room.name ?? '—'}</h3>
            ...
            <div className="flex items-end justify-end mt-2 pt-4 border-t border-adminGray-100/80">
              <PermissionGate resource="booking_rooms" action="update">
                <Button variant="admin" size="sm" onClick={() => onEdit?.(room)} className="bg-adminGreen-600 hover:opacity-90 text-white shadow-sm h-8 px-4 text-sm gap-1.5 rounded-md transition-opacity">
        ...
        <TabsContent value="positions" className="p-4 m-0 border-none outline-none">
          {positions.length === 0 ? (
            <div className="py-8 text-center text-adminGray-600 text-sm">Chưa có vị trí nào được thiết lập trong phòng này</div>
          ) : (
            <div className="rounded-md border border-adminGray-100 overflow-x-auto w-full">
              <table className="w-full text-left text-sm min-w-[500px]">
                ...
                <tbody className="divide-y divide-adminGray-100 bg-white">
                  {positions.map((pos) => (
                    <tr key={pos.id} className="hover:bg-adminGray-50/50 transition-colors">
                      <td className="py-2.5 px-4 font-semibold text-adminGray-600 text-center">{pos.sortOrder ?? '-'}</td>
                      ...
                      <td className="py-2.5 px-4 text-adminGray-600 truncate max-w-[200px]" title={pos.note}>{pos.note || '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
...
function DetailField({ label, value }: { label: string; value: string | null | undefined }) {
  return (
    <div className="py-3.5 border-b border-adminGray-100/80 last:border-b-0 group">
      ...
      <p className="text-sm font-medium text-adminInk truncate">{value || '—'}</p>
    </div>
  )
}

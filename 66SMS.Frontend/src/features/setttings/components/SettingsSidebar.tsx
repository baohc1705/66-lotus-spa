...
import {
  SETTINGS_MENU,
  SETTINGS_MENU_CHEVRON,
  type SettingsMenuItem,
  type SettingsMenuLink,
...
function itemHasActiveChild(pathname: string, item: SettingsMenuItem) {
  if (!item.children) return false;
  return item.children.some((child: SettingsMenuLink) => isPathActive(pathname, child.path));
}

function findOpenLabel(pathname: string): string | null {
...
export function SettingsSidebar(props: SettingsSidebarProps) {
  ...
  const Chevron = SETTINGS_MENU_CHEVRON;
  const [openLabel, setOpenLabel] = useState<string | null>(() => findOpenLabel(pathname));

  useEffect(() => {
    setOpenLabel(findOpenLabel(pathname));
  }, [pathname]);
      ...
      <div className="scrollbar-sidebar h-full w-full overflow-y-auto overflow-x-hidden">
        <div className="app-sidebar__inner px-4 pb-4 pt-0.5">
          <ul className="vertical-nav-menu relative m-0 list-none p-0">
            {SETTINGS_MENU.map((section: SettingsMenuSection) => (
              <li key={section.heading} className="list-none">
                ...
                <ul className="m-0 list-none p-0">
                  {section.items.map((item: SettingsMenuItem) => {
                    const Icon = item.icon;
                    const hasChildren = !!item.children?.length;
                    const isOpen = openLabel === item.label;
                    const parentActive = itemHasActiveChild(pathname, item);
                    const isExternal = !!item.path?.startsWith("http");
                    const isActiveLeaf = item.path ? isPathActive(pathname, item.path) : false;
                    ...
                    if (hasChildren) {
                      return (
                        <li
                          key={item.label}
                          className={isOpen || parentActive ? "mm-active" : undefined}
                        >
                          <button
                            type="button"
                            title={item.label}
                            ...
                            className={
                              ...
                              "hover:bg-blue-50 " +
                              (isOpen || parentActive ? "font-bold" : "font-normal")
                            }
                          >
                          ...
                          >
                            ...
                            {item.children!.map((child: SettingsMenuLink) => {
                      ...
                      return (
                        <li key={item.label}>
                          <a
                            ...
                            title={item.label}
                            className={linkBase + " text-sm hover:bg-blue-50"}
                          >
                            <Icon className={iconClass + "text-kit-muted"} />
                            <span
                              className={
                      ...
                      <li key={item.label}>
                        ...
                        >
                          <Icon
                            className={
                              iconClass +
                              (isActiveLeaf ? "text-kit-primary opacity-80" : "text-kit-muted")
                            }
                          />
                          <span
                            className={

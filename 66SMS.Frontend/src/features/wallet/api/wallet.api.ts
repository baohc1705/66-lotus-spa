...
export const getAdminWalletTransactions = async (walletId: number) => {
  const response = await axiosInstance.get<Result<AdminWalletTransactionDto[]>>(`${API.admin.wallets}/${walletId}/transactions`)
  return response.data
}

export const addManualTransaction = async (walletId: number, data: { amount: number; note: string }) => {
  const response = await axiosInstance.post<Result<object>>(`${API.admin.wallets}/${walletId}/transaction`, data)
  return response.data
}

export const getMyWallet = async () => {
  const { data } = await axiosInstance.get<Result<{ balance: number }>>(API.users.meWallet)
  return data
}

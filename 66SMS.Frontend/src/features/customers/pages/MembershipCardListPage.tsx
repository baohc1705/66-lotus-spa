...
import {
  MoreHorizontal, Eye, Pencil, CreditCard,
...
export function MembershipCardListPage() {
  ...
  const { data: cardsResult, isLoading, isFetching } = useMembershipCards({
    pageIndex,
    pageSize,
  ...
  const columns = useMemo<ColumnDef<MembershipCardDto>[]>(() => [
    ...
    },
    {
      ...
      header: () => (
        <button onClick={() => handleSort('cardCode')} className="flex items-center gap-1.5 hover:text-lotus-leaf transition-colors">
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold text-lotus-deep">{row.original.cardCode}</span>
        </div>
      ),
      size: 150,
    },
    {
      ...
      header: 'Khách hàng',
      cell: ({ row }) => <span className="font-semibold text-lotus-deep/80">{row.original.customerName ?? '—'}</span>,
      size: 180,
    },
    {
      accessorKey: 'tierName',
      header: 'Loại thẻ',
      cell: ({ row }) => <span className="text-lotus-deep/80 font-medium">{row.original.tierName ?? '—'}</span>,
      size: 120,
    },
    {
      accessorKey: 'issuedAt',
      header: 'Ngày cấp',
      cell: ({ row }) => <span className="text-lotus-deep/80 text-[13px]">{row.original.issuedAt ? new Date(row.original.issuedAt).toLocaleDateString('vi-VN') : '—'}</span>,
      size: 110,
    },
    {
      accessorKey: 'expiresAt',
      header: 'Hết hạn',
      cell: ({ row }) => <span className="text-lotus-deep/80 text-[13px]">{row.original.expiresAt ? new Date(row.original.expiresAt).toLocaleDateString('vi-VN') : 'Vĩnh viễn'}</span>,
      size: 110,
    },
    {
      accessorKey: 'status',
      header: 'Trạng thái',
      cell: ({ row }) => <StatusBadge status={row.original.status} statusMap={CARD_STATUS_MAP} />,
      size: 120,
    },
    {
      id: 'actions',
      ...
      <motion.div variants={itemVariants} className="bg-white/70 backdrop-blur-md rounded-admin border border-stone-200/30 overflow-hidden">
        ...
        <DataTable
          ...
          renderSubComponent={({ row }) => (
            row.original.id ? <MembershipCardDetailExpanded cardId={row.original.id} onEdit={(card) => setEditCard(card)} /> : null
          )}
          emptyState={
            <div className="flex flex-col items-center gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-lotus-deep">Chưa có thẻ thành viên</p>
                <p className="text-[12px] text-lotus-stone mt-0.5">Thẻ sẽ tự động được tạo khi khách hàng mới được đăng ký.</p>
              </div>
            </div>
          }
          pagination={

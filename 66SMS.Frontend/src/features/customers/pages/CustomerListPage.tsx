...
export function CustomerListPage() {
  ...
  // Data fetching
  const { data: customersResult, isLoading, isFetching } = useCustomers({
    pageIndex,
    pageSize,
  ...
  // Selection logic
  const currentPageIds = useMemo(() => customers.map(c => c.id).filter((id): id is number => id !== undefined), [customers])
  ...
  const columns = useMemo<ColumnDef<CustomerDto>[]>(() => [
        ...
        return (
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-lg bg-amber-500/10 flex items-center justify-center shrink-0 overflow-hidden">
              {cust.image ? (
                <img src={cust.image} alt="" className="w-8 h-8 object-cover" />
              ) : (
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return <span className="font-semibold text-lotus-deep">{points ?? 0}</span>
      },
      size: 80,
    },
    {
      accessorKey: 'source',
      header: 'Nguồn',
      cell: ({ row }) => <span className="text-lotus-deep/70">{row.original.source ?? '—'}</span>,
      size: 110,
    },
    {
      accessorKey: 'status',
      header: 'Trạng thái',
      cell: ({ row }) => <StatusBadge status={row.original.status} statusMap={CUSTOMER_STATUS_MAP} />,
      size: 120,
    },
    {
      id: 'actions',
          ...
          <div onClick={(e) => e.stopPropagation()}>
            <DropdownMenu>
              ...
              <DropdownMenuContent>
                ...
                <PermissionGate resource="customers" action="delete" role="admin">
                  <DropdownMenuSeparator />
                  <DropdownMenuItem variant="destructive" onClick={() => setDeleteTarget(cust)}>
      ...
      <motion.div variants={itemVariants} className="bg-white/70 backdrop-blur-md rounded-admin border border-stone-200/30 overflow-hidden">
        ...
        <DataTable
          ...
          renderSubComponent={({ row }) => (
            row.original.id ? <CustomerDetailExpanded customerId={row.original.id} onEdit={(cust) => setEditCustomer(cust)} /> : null
          )}
          emptyState={
            <div className="flex flex-col items-center gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-lotus-deep">Chưa có khách hàng</p>
                <p className="text-[12px] text-lotus-stone mt-0.5">Thêm khách hàng mới để bắt đầu quản lý.</p>
              </div>
              <PermissionGate resource="customers" action="create" role="admin">

...
export function MembershipCardFormDialog({ open, onOpenChange, card }: MembershipCardFormDialogProps) {
  ...
  const tiers = useMemo(() => tiersResult?.data?.items ?? [], [tiersResult?.data?.items])

  const form = useForm<MembershipCardFormValues>({
    resolver: zodResolver(updateMembershipCardSchema) as Resolver<MembershipCardFormValues>,
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-5">
            <FormField label="Mã thẻ *" error={errors.cardCode?.message}>
              <Input {...register('cardCode')} placeholder="Nhập mã thẻ" className="h-9 text-[13px]" />
            </FormField>
            
            <FormField label="Loại thẻ" error={errors.membershipTierId?.message}>
              <Select
            ...
            <FormField label="Ngày cấp" error={errors.issuedAt?.message}>
              <Input 
                type="datetime-local" 
                defaultValue={getDatetimeLocalFormat(watch('issuedAt'))}
                onChange={(e) => handleDateChange('issuedAt', e.target.value)}
                className="h-9 text-[13px]" 
              />
            </FormField>

            <FormField label="Ngày hết hạn" tooltip="Để trống nếu thẻ có giá trị vĩnh viễn" error={errors.expiresAt?.message}>
              <Input 
                type="datetime-local" 
                defaultValue={getDatetimeLocalFormat(watch('expiresAt'))}
...
function getDefaultValues(card?: MembershipCardDto | null): MembershipCardFormValues {
  if (card) {
    return {

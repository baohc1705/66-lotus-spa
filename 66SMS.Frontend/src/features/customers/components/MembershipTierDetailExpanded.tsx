...
export function MembershipTierDetailExpanded({ tierId, onEdit }: MembershipTierDetailExpandedProps) {
  ...
  if (!tier) {
    return <div className="p-6 text-center text-lotus-stone text-sm bg-stone-50/30">Không tìm thấy thông tin loại thẻ</div>
  }
      ...
      <div className="flex flex-col gap-5">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            <h3 className="text-base font-bold text-lotus-deep truncate">{tier.name}</h3>
            <p className="text-[12px] font-medium text-lotus-stone mt-0.5">
              Trạng thái: <span className={tier.status === 1 ? 'text-green-600' : 'text-amber-600'}>{STATUS_MAP[tier.status] ?? 'Không rõ'}</span>
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <DetailCard icon={HandCoins} label="Chi tiêu tối thiểu" value={formatCurrency(tier.minSpending)} />
          <DetailCard icon={Percent} label="Giảm giá (%)" value={`${tier.discountPercent ?? 0}%`} />
          <DetailCard icon={Crown} label="Hệ số điểm" value={`x${tier.pointMultiplier}`} />
          <div className="flex flex-col">
            <p className="text-[12px] font-medium text-lotus-stone mb-1">Ngày tạo</p>
            <p className="text-[13px] text-lotus-deep font-medium">{tier.createdAt || '—'}</p>
          </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mt-2 pt-4 border-t border-stone-100/80">
          <div className="flex-1">
            <p className="text-[12px] font-medium text-lotus-stone mb-1">Quyền lợi chi tiết</p>
            <p className="text-[13px] text-lotus-deep whitespace-pre-wrap">{tier.benefits || 'Chưa cập nhật quyền lợi'}</p>
          </div>
          <PermissionGate resource="customers" action="update">
            <Button variant="admin" size="sm" onClick={() => onEdit?.(tier)} className="bg-lotus-leaf hover:opacity-90 text-white shadow-sm h-8 px-4 text-[13px] gap-1.5 rounded-md transition-opacity shrink-0">

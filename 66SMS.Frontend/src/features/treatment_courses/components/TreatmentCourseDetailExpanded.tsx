...
export function TreatmentCourseDetailExpanded({ courseId, onEdit }: Props) {
  ...
  if (isLoading) {
    return (
      <div className="p-4 text-[13px] text-lotus-stone animate-pulse">
        Đang tải chi tiết...
      </div>
    );
  }
  ...
  if (!course) {
    return (
      <div className="p-4 text-[13px] text-red-400">
    ...
    <div className="px-6 py-4 bg-lotus-cream/30 border-t border-stone-100">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-3">
          <span className="text-[12px] font-semibold text-lotus-deep">
            {course.name}
          </span>
          ...
          <span className="text-[12px] text-lotus-stone">
            ...
            <strong className="text-lotus-deep">
              {(course.sellingPrice ?? 0).toLocaleString("vi-VN")}đ
            </strong>
          </span>
        </div>
      ...
      {course.description && (
        <p className="mt-3 text-[12px] text-lotus-stone italic">
          {course.description}
        </p>
      )}
    </div>

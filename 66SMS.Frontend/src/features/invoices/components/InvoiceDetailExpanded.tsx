...
export function InvoiceDetailExpanded({ invoiceId, onCancel }: Props) {
  ...
  if (isLoading) return <div className="p-4 text-lotus-admin-lg text-lotus-stone animate-pulse">Đang tải chi tiết...</div>
  if (!inv) return <div className="p-4 text-lotus-admin-lg text-red-400">Không tải được chi tiết.</div>
    ...
    <div className="px-6 py-4 bg-lotus-cream/30 border-t border-stone-100">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-3 flex-wrap">
          <span className="text-lotus-admin-md font-semibold text-lotus-deep font-mono">{inv.invoiceCode}</span>
          <span className="text-lotus-admin-base text-lotus-stone">•</span>
          <span className="text-lotus-admin-md text-lotus-stone">{inv.customerName ?? 'Khách vãng lai'}{inv.customerPhone ? ` (${inv.customerPhone})` : ''}</span>
          {inv.salonName && <><span className="text-lotus-admin-base text-lotus-stone">•</span><span className="text-lotus-admin-md text-lotus-stone">{inv.salonName}</span></>}
        </div>
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" onClick={() => window.print()} className="text-lotus-admin-md gap-1.5">
            <Printer className="w-3.5 h-3.5" /> In
          </Button>
          {canCancel && inv.id && (
            <PermissionGate resource="invoices" action="update">
              <Button variant="outline" size="sm" onClick={() => onCancel(inv.id!)}
                className="text-lotus-admin-md gap-1.5 text-red-500 hover:text-red-600 hover:bg-red-50">
      ...
      {items.length === 0 ? (
        <p className="text-lotus-admin-lg text-lotus-stone">Không có dòng nào.</p>
      ) : (
            ...
            <tbody>
              {items.map((item: InvoiceItemDto) => (
                <tr key={item.id} className="border-b border-stone-100 last:border-0 hover:bg-stone-50/50">
                  ...
                  <td className="py-2 pr-4 text-right text-lotus-stone">{formatCurrency(item.unitPrice)}</td>
                  <td className="py-2 pr-4 text-center text-lotus-stone">{item.quantity ?? 1}</td>
                  <td className="py-2 pr-4 text-right text-lotus-stone">{formatCurrency(item.discountAmount)}</td>
                  <td className="py-2 pr-4 text-right font-semibold text-lotus-deep">{formatCurrency(item.lineTotal)}</td>
      ...
      <div className="mt-4 flex justify-end">
        <div className="w-full sm:w-72 text-lotus-admin-md space-y-1">
          ...
          <div className="flex justify-between border-t border-stone-200 pt-1 text-lotus-admin-lg">
            ...
            <strong className="text-lotus-leaf">{formatCurrency(inv.totalAmount)}</strong>
          </div>
      ...
      {inv.note && <p className="mt-3 text-lotus-admin-md text-lotus-stone italic">Ghi chú: {inv.note}</p>}
    </div>
  )
}

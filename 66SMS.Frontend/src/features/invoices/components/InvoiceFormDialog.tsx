...
export function InvoiceFormDialog({ open, onOpenChange }: Props) {
  ...
  const subTotal = (watchedItems ?? []).reduce((sum, it) => {
    ...
    const line =
      unit * Number(it.quantity || 0) - Number(it.discountAmount || 0);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[960px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Chọn khách hàng (hoặc khách vãng lai), thêm dịch vụ/sản phẩm/liệu
            trình rồi thanh toán.
          </DialogDescription>
        </DialogHeader>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              <FormField label="Khách hàng (có sẵn)">
                <SearchableSelect
                  ...
                  onValueChange={(v) =>
                    setValue("customerId", v ? Number(v) : undefined)
                  }
              ...
              <FormField label="Chi nhánh / Salon">
                <SearchableSelect
                  ...
                  onValueChange={(v) =>
                    setValue("salonId", v ? Number(v) : undefined)
                  }
                  options={salons.map((s: SalonDTO) => ({
                    value: String(s.id ?? ""),
                    label: `${s.name ?? ""} — ${s.code ?? ""}`,
                  }))}
                ...
                return (
                  ...
                  >
                    <div className="col-span-2">
                      <Label className="text-xs text-adminInk/70 mb-1 block">
                        Loại *
                      </Label>
                      <Select
                        value={
                        ...
                        onValueChange={(v) => {
                          setValue(`items.${index}.itemType`, Number(v));
                    ...
                    <div className="col-span-4">
                      ...
                      <SearchableSelect
                        ...
                        onValueChange={(v) =>
                          setValue(`items.${index}.refId`, v ? Number(v) : 0)
                        }
                      ...
                      {errors.items?.[index]?.refId && (
                        <p className="text-xs text-state-danger-text mt-0.5">
                          {errors.items[index]?.refId?.message}
                        </p>
                      )}
                    </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
              ...
              <FormField label="Áp dụng hạng thành viên">
                <Select
                  ...
                  onValueChange={(v) =>
                    setValue("applyMembershipDiscount", v === "1")
                  }
                >
                  <AdminSelectTrigger>
                    <SelectValue />
  ...
  return (
    <div className={`space-y-1 ${className ?? ""}`}>
      <Label className="text-xs font-semibold text-adminInk/80">
        {cleanLabel}
        {isRequired && <span className="text-state-danger-text ml-0.5">*</span>}
      </Label>
      {children}
      {error && (

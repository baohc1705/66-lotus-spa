...
const itemSchema = z.object({
  itemType: z.coerce.number().min(1, VALIDATION_MSG.selectRequired("loại")).max(3),
...
export const invoiceSchema = z.object({
  ...
  customerName: z.string().max(200, VALIDATION_MSG.max(200)).optional().or(z.literal("")),
  customerPhone: z.string().max(20, VALIDATION_MSG.max(20)).optional().or(z.literal("")),
  ...
  paymentMethod: z.coerce.number().min(1, VALIDATION_MSG.selectRequired("phương thức thanh toán")).max(4),
  paidAmount: z.coerce.number().min(0, VALIDATION_MSG.min(0)).optional(),
  transactionId: z.string().max(100, VALIDATION_MSG.max(100)).optional().or(z.literal("")),
  note: z.string().max(500, VALIDATION_MSG.max(500)).optional().or(z.literal("")),

    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[850px]">
        <DialogHeader>
          ...
          <DialogDescription>
            {isEdit ? `Cập nhật thông tin nhân viên ${employee?.fullName ?? ''}` : 'Điền thông tin để tạo nhân viên mới'}
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
          ...
          <FormSection icon={User} title="Thông tin cá nhân">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-5">
              <FormField label="Họ tên *" tooltip="Vui lòng nhập họ và tên đầy đủ của nhân viên" error={errors.fullName?.message}>
              ...
              <FormField label="CMND/CCCD" error={errors.nationalId?.message}>
                <Input {...register('nationalId')} placeholder="012345678901" className="h-9 text-[13px]" />
              </FormField>
              <FormField label="Địa chỉ" error={errors.fullAddress?.message}>
                <Input {...register('fullAddress')} placeholder="123 Đường ABC, Quận 1, TP.HCM" className="h-9 text-[13px]" />
              </FormField>
            </div>
          </FormSection>
          ...
          <FormSection icon={Briefcase} title="Thông tin công việc">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-5">
              <FormField label="Ngày vào làm" error={errors.hireDate?.message}>
                <Input {...register('hireDate')} type="date" className="h-9 text-[13px]" />
              </FormField>
              ...
              <FormField label="Lương cơ bản" error={errors.basicSalary?.message}>
                <Input
            ...
            <FormSection icon={KeyRound} title="Tài khoản đăng nhập">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-5">
                ...
                <FormField label="Email *" tooltip="Địa chỉ email hợp lệ (ví dụ: user@example.com)" error={errors.email?.message}>
                  <Input {...register('email')} type="email" placeholder="nv@hoasenspa.com" className="h-9 text-[13px]" />
                </FormField>
                <FormField label="Mật khẩu *" tooltip="Mật khẩu phải có ít nhất 6 ký tự, bao gồm chữ hoa, chữ thường và số" error={(errors as Record<string, { message?: string }>).password?.message}>
...
function FormField({ label, error, tooltip, children }: { label: string; error?: string; tooltip?: string; children: React.ReactNode }) {

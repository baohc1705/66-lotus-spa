...
import {
  ...
  ArrowUpDown, Users, ArrowUp, ArrowDown, X
} from 'lucide-react'
...
import { EmployeeDetailExpanded } from '../components/EmployeeDetailExpanded'
...
export function EmployeeListPage() {
  ...
  // Data fetching
  const { data: employeesResult, isLoading, isFetching } = useEmployees({
    pageIndex,
    pageSize,
  ...
  const paged = employeesResult?.data
  const employees = useMemo(() => paged?.items ?? [], [paged?.items])
  const totalCount = paged?.totalCount ?? 0

  // Selection logic
  const currentPageIds = useMemo(() => employees.map(e => e.id).filter((id): id is number => id !== undefined), [employees])
  const isAllSelected = currentPageIds.length > 0 && currentPageIds.every(id => selectedRowIds.has(id))
  ...
  const columns = useMemo<ColumnDef<EmployeeDto>[]>(() => [
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <Checkbox
            checked={emp.id !== undefined && selectedRowIds.has(emp.id)}
            onCheckedChange={(checked) => {
    ...
    },
    {
      ...
      cell: ({ row }) => <Badge variant="outline" size="sm" className="text-[10px] font-mono">{row.original.code ?? '—'}</Badge>,
      size: 100,
    },
    {
      accessorKey: 'fullName',
      header: () => (
        <button onClick={() => handleSort('fullname')} className="flex items-center gap-1.5 hover:text-lotus-leaf transition-colors">
        ...
        return (
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-lg bg-lotus-leaf/10 flex items-center justify-center shrink-0 overflow-hidden">
              {emp.image ? (
                <img src={emp.image} alt="" className="w-8 h-8 object-cover" />
              ) : (
    ...
    },
    {
      accessorKey: 'phone',
      header: 'SĐT',
      cell: ({ row }) => <span className="text-lotus-deep/80">{row.original.phone ?? '—'}</span>,
      size: 110,
    },
    {
      accessorKey: 'email',
      header: () => (
        <button onClick={() => handleSort('email')} className="flex items-center gap-1.5 hover:text-lotus-leaf transition-colors">
          Email <SortIcon column="email" />
        </button>
      ),
      cell: ({ row }) => <span className="text-lotus-deep/70">{row.original.email ?? '—'}</span>,
      size: 180,
    },
    {
      accessorKey: 'gender',
      header: 'Giới tính',
      cell: ({ row }) => <span className="text-lotus-deep/70">{GENDER_MAP[row.original.gender ?? ''] ?? '—'}</span>,
      size: 90,
    },
    {
      accessorKey: 'contractType',
      header: 'Loại HĐ',
      cell: ({ row }) => <span className="text-lotus-deep/70">{row.original.contractType ?? '—'}</span>,
      size: 110,
    },
    {
      accessorKey: 'basicSalary',
      header: 'Lương',
      cell: ({ row }) => <span className="font-semibold text-lotus-deep">{formatCurrency(row.original.basicSalary)}</span>,
      size: 100,
    },
    {
      accessorKey: 'status',
      header: 'Trạng thái',
      cell: ({ row }) => <StatusBadge status={row.original.status} statusMap={EMPLOYEE_STATUS_MAP} />,
      size: 120,
    },
    {
      id: 'actions',
          ...
          <div onClick={(e) => e.stopPropagation()}>
            <DropdownMenu>
              ...
              <DropdownMenuContent>
                ...
                <PermissionGate resource="employees" action="delete" role="admin">
                  <DropdownMenuSeparator />
                  <DropdownMenuItem variant="destructive" onClick={() => setDeleteTarget(emp)}>
  ...
  ], [headerChecked, selectedRowIds, currentPageIds, pageIndex, pageSize, handleSort, SortIcon])
      ...
      <motion.div variants={itemVariants} className="bg-white/70 backdrop-blur-md rounded-admin border border-stone-200/30 overflow-hidden">
          ...
          >
            {selectedRowIds.size > 0 && (
              <div className="flex items-center gap-2 mr-auto text-[13px] text-lotus-deep font-medium bg-lotus-cream/50 px-3 py-1.5 rounded-lg border border-stone-200/50">
                ...
                <button onClick={() => setSelectedRowIds(new Set())} className="text-lotus-stone hover:text-lotus-deep ml-1 transition-colors" title="Bỏ chọn tất cả">
            ...
            <PermissionGate resource="employees" action="create" role="admin">
              <Button
                variant="admin"
                size="sm"
        ...
        <DataTable
          ...
          renderSubComponent={({ row }) => (
            row.original.id ? <EmployeeDetailExpanded employeeId={row.original.id} onEdit={(emp) => setEditEmployee(emp)} /> : null
          )}
          emptyState={
            <div className="flex flex-col items-center gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-lotus-deep">Chưa có nhân viên</p>
                <p className="text-[12px] text-lotus-stone mt-0.5">Thêm nhân viên mới để bắt đầu quản lý.</p>
              </div>
              <PermissionGate resource="employees" action="create" role="admin">

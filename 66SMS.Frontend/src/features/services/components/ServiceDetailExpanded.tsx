...
export function ServiceDetailExpanded({ serviceId, onEdit }: ServiceDetailExpandedProps) {
  ...
  if (!service) {
    return <div className="p-6 text-center text-lotus-stone text-sm bg-stone-50/30">Không tìm thấy thông tin dịch vụ</div>
  }
      ...
      <Tabs defaultValue="info" className="w-full flex-col">
        ...
        <div className="px-4 pt-2 sticky top-0 bg-stone-50/95 backdrop-blur-sm z-10">
          <TabsList className="h-10 border-b border-stone-200/80 justify-start rounded-none bg-transparent p-0 flex flex-nowrap overflow-x-auto overflow-y-hidden hide-scrollbar">
            ...
            <TabsTrigger value="products" className="relative h-10 rounded-none border-0 border-b-2 border-transparent bg-transparent px-3 pb-2 pt-2 text-[13px] font-medium text-lotus-stone hover:text-lotus-leaf/80 data-[state=active]:border-lotus-leaf data-[state=active]:text-lotus-leaf data-[state=active]:bg-transparent data-[state=active]:shadow-none focus-visible:ring-0 focus-visible:outline-none whitespace-nowrap transition-colors">Sản phẩm tiêu hao ({products.length})</TabsTrigger>
          </TabsList>
        </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                <h3 className="text-base font-bold text-lotus-deep truncate">{service.name ?? '—'}</h3>
                <p className="text-[12px] text-lotus-stone mt-0.5">
                  Mã: {service.code || '—'} · Thời gian: {service.durationMins || 0} phút
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-0">
              <div className="flex flex-col">
                <DetailField label="Nhóm dịch vụ" value={service.categoryName} />
                <DetailField label="Mô tả ngắn" value={service.description} />
                <DetailField label="Giá cơ bản" value={service.costPrice ? `${service.costPrice.toLocaleString()} ₫` : '—'} />
              </div>
              <div className="flex flex-col">
                <DetailField label="Hoa hồng" value={service.commissionRate != null ? `${service.commissionRate}%` : '—'} />
                <DetailField label="Nội dung" value={service.content} />
                <DetailField label="Giá bán" value={service.sellingPrice ? `${service.sellingPrice.toLocaleString()} ₫` : '—'} />
              </div>
            </div>
            ...
            <div className="flex items-end justify-end mt-2 pt-4 border-t border-stone-100/80">
              <PermissionGate resource="services" action="update">
                <Button variant="admin" size="sm" onClick={() => onEdit?.(service)} className="bg-lotus-leaf hover:opacity-90 text-white shadow-sm h-8 px-4 text-[13px] gap-1.5 rounded-md transition-opacity">
        ...
        <TabsContent value="products" className="p-4 m-0 border-none outline-none">
          {products.length === 0 ? (
            <div className="py-8 text-center text-lotus-stone text-sm">Chưa có sản phẩm tiêu hao nào được cấu hình cho dịch vụ này</div>
          ) : (
            <div className="rounded-md border border-stone-200 overflow-hidden">
              <table className="w-full text-left text-[13px]">
                ...
                <tbody className="divide-y divide-stone-100 bg-white">
                  {products.map((prod) => (
                    <tr key={prod.id || prod.productId} className="hover:bg-stone-50/50 transition-colors">
                      ...
                      <td className="py-2.5 px-4 font-semibold text-lotus-stone text-center">{prod.quantityUsed ?? '-'}</td>
                      <td className="py-2.5 px-4">
                        <StatusBadge status={prod.status?.toString()} statusMap={PRODUCT_STATUS_MAP} />
                      </td>
                      <td className="py-2.5 px-4 text-lotus-stone truncate max-w-[200px]" title={prod.note}>{prod.note || '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
